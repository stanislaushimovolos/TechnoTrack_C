...
#include "../Fileworking/fileworking.h"
#include "Tree.h"
...
Node *TreeSearcherRec (Node *node, char **target) {
	...
	if (strcmp (*target, node->content) == 0) {
		TargetFlag = 1;
		return node;
	}
...
int dumpRecNode (const Node *const n, int *NodeCounter, FILE *outPictureFile1) {
	fprintf (outPictureFile1, "Node%p [shape = record,  color = red, label = \"{ { %s| '%p' } | { Parent| '%p' } | ",
	         n, n->content, n, n->Parent);
...
int dumpTreePicture (const Tree *const tree, const char *outFileName) {
	...
	system ("C:\\[eq\\bin\\dot -Tpng C:"
			        "\\Users\\Tom\\CLionProjects\\CPU\\Akinator\\picture\\dotDump.txt "
			        "-o C:\\Users\\Tom\\CLionProjects\\CPU\\Akinator\\picture\\dump.png");
...
int printTree (const Tree *const tree, const char *outFileName) {
	...
	system ("C:\\[eq\\bin\\dot -Tpng C:"
			        "\\Users\\Tom\\CLionProjects\\CPU\\Akinator\\picture\\dotPrint.txt "
			        "-o C:\\Users\\Tom\\CLionProjects\\CPU\\Akinator\\picture\\print.png");
